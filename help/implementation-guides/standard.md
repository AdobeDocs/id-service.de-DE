...
* Experience Cloud-Organisations-ID (wird automatisch ausgefüllt, wenn eine Verknüpfung mit der Experience Cloud hergestellt wird)
* Analytics-Tracking-Server (sicher und nicht sicher)
...
* [Dynamic Tag Management – Erste Schritte](https://docs.adobe.com/content/help/en/dtm/using/getting-started/get-started.html)
...
Kunden müssen einen CNAME bereitstellen oder `*.sc.omtrdc` für die [regionale Datenerfassung (Regional Data Collection, RDC)](https://docs.adobe.com/content/help/en/analytics/technotes/rdc/regional-data-collection.html) verwenden. Erfragen Sie die jeweiligen RDC-Einstellungen bei Ihrem [!DNL Adobe]-Berater.
...
Führen Sie die folgenden Schritte aus, um den ID-Dienst mit dem dynamischen Tag-Management (DTM) zu implementieren.
...
* Erstellen Sie eine Webeigenschaft in DTM. Weitere Informationen finden Sie in der Dokumentation zu DTM [Create a Web Property](https://docs.adobe.com/content/help/en/dtm/using/admin/web-property.html) .
...
Die Einstellungen stehen nach dem Hinzufügen und Speichern des ID-Dienstes als ein Tool im Dynamic Tag Management (DTM) zur Verfügung. You can also access these settings by clicking the gear icon from the [!UICONTROL Installed Tools] section of your DTM web property.
...
Der Name des für die Analytics-Datenerfassung verwendeten Tracking-Servers. Dies ist die Domäne, auf der die Bildanforderung und das Cookie geschrieben werden (z. B. `http://site.omtrdc.net`).
...
Der Name des sicheren Tracking-Servers, der für die Analytics-Datenerfassung verwendet wird. Dies ist die Domäne, auf der die Bildanforderung und das Cookie geschrieben werden (z. B. `https://site.omtrdc.net`).
...
Der Wert sollte ein Datenelement sein, das die Benutzer-ID enthält. Datenelemente sind geeignete Container für dynamische Werte wie IDs eines kundenspezifischen internen Systems.

**Authentifizierungsstatus**

Optionen, die Besucher je nach Authentifizierungsstatus definieren oder identifizieren (z. B. angemeldet, abgemeldet). Siehe [Kunden-IDs und Authentifizierungsstatus](../reference/authenticated-state.md).
