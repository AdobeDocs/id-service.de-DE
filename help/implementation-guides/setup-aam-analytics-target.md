...
title: Implementieren des Adobe-Besucher-ID-Service für Analytics, Audience Manager und Target
...
# Implementieren des Adobe-Besucher-ID-Service für Analytics, Audience Manager und Target {#implement-the-experience-cloud-id-service-for-analytics-audience-manager-and-target}
...
>[!IMPORTANT]
>
>Lesen Sie die [ für den Besucher-ID-](../reference/requirements.md) , bevor Sie beginnen, und beachten Sie die folgenden Anforderungen für diese Implementierung:
>
...
1. Gehe zu **[!UICONTROL Admin > Code Manager]**.
...
Wenn einer dieser Anwendungsfälle auf Ihre Situation zutrifft, bitten Sie [Kundenunterstützung](https://helpx.adobe.com/de/marketing-cloud/contact-support.html), eine vorübergehende Übergangsphase einzurichten. Übergangsphasen können bis zu 180 Tage dauern. Bei Bedarf kann eine Übergangsphase verlängert werden.
...
Die CX Enterprise-Lösungen in dieser Implementierung geben IDs in Form von Schlüssel-Wert-Paaren zurück. Jede Lösung verwendet unterschiedliche Schlüssel (z. B. die Analytics-SDID vs. die Target-mboxMCSDID) für dieselbe ID. Laden Sie zum Testen Ihrer Implementierung Ihre Seiten in einer Entwicklungsumgebung. Verwenden Sie Ihre Browserkonsole oder Software, die HTTP-Anforderungen und -Antworten überwacht, um die unten aufgeführten IDs zu überprüfen. Der Besucher-ID-Dienst wurde korrekt implementiert, wenn die unten aufgeführten Schlüssel-Wert-Paare dieselben ID-Werte zurückgeben.
...
* ECID mit dem Adobe-Debugger oder dem von Ihnen bevorzugten Debugging-Tool.

Weitere Prüfungen, mit denen Sie feststellen können, ob der Besucher-ID-Dienst ordnungsgemäß funktioniert, finden Sie unter [Testen und Überprüfen des Besucher-ID-Dienstes](../implementation-guides/test-verify.md).
...
Wenn eine mboxMCAVID angezeigt wird, überprüfen Sie deren Wert mit der Analytics-AID. Diese Werte sind identisch, wenn der Besucher-ID-Dienst korrekt implementiert wurde.
